...
if kinetics.size == 1
	...
	i = 0 #pointer
		if carton[i..i+7] != "\x00\x00\x00\x00\x00\x00\xF0\x3F"
			puts carton[i..i+7].class 
			raise "sample name head not recog #{display_byte(carton[i..i+7])}, expecting #{display_byte("\x00\x00\x00\x00\x00\x00\xF0\x3F")}"
		end
		i += 8
...
cartons = kinetics.split("Thermo Scientific DataCarton")
puts carton.size
